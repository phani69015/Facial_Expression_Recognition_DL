...
def ann_model_rmsprop():
    ...
    adam_optimizer = optimizers.RMSprop(learning_rate=0.001)  # Adjust learning rate
...
model=ann_model_rmsprop()
checkpoint = ModelCheckpoint("rmsprop_weights.h5", monitor='val_accuracy', verbose=1, save_best_only=True, mode='max')
...
with open('rmsprop_history.pkl', 'wb') as file:
